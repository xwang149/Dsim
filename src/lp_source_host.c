...
//	printf("reach here!\n");
	for(int i = 0; i<WINDOW; i++){
		...
		for (int c=1; c<=conn_slots;c++){
	    	double t_est = (double) job->inputsize/(c*job->bandwidth/max_slot);
	    	double t_wait = (double) now - job->stats.created;
	    	double resp = (double) t_wait + t_est;
            double deadtime = job->t_dead;
            if (resp < deadtime)
            	U[i][c] = (double) 100 - 100*(deadtime - resp)/deadtime;
            else
            	U[i][c] = 0;
//            printf("t_est=%12f;resp=%12f;dead=%12f;U=%12f\n",t_est,resp,deadtime,U[i][c]);
            printf("%12f", U[i][c]);
		}
		printf("\n");
	}
...
static void calc_delta(double leftDelta[], double rightDelta[], int conn[], double U[][MAX_CONN+1], int conn_slots){
	for(int i=0; i<WINDOW; i++){
		printf("%12d,%12d\t", conn[i],i);
		if (conn[i] == 0)
			leftDelta[i] = MAX_NUM;
		else
			leftDelta[i] = U[i][conn[i]-1] - U[i][conn[i]];

		if (conn[i] == conn_slots)
			rightDelta[i] = 0 - MAX_NUM;
		else
			rightDelta[i] = U[i][conn[i]] - U[i][conn[i]+1];
